...
We will see a quick setup to configure the ``invidious`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``invidious`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``invidious`` formulas under ``/srv/salt/invidious/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/invidious/parameters/dns:domain/
    ln -s dns:domain /srv/salt/invidious/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/invidious/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/invidious/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``invidious`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``invidious`` formulas under ``/srv/salt/invidious/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/invidious/parameters/roles
...
We create a configuration for the role ``invidious/server`` in ``/srv/salt/invidious/parameters/roles/invidious/server.yaml``:
...
We create another configuration for the role ``invidious/client`` in ``/srv/salt/invidious/parameters/roles/invidious/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles invidious/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles invidious/client
...
.. code-block:: console
    ...
    mv /srv/salt/invidious/parameters/dns:domain/  '/srv/salt/invidious/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-invidious-as-json:
      test.nop:
        - name: {{ invidious | json }}
...
Here is an example based on `template-formula/invidious/config/file.sls`_:
    ...
    invidious-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='invidious-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ invidious.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``invidious`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ invidious['winner'] }}
...
.. _template-formula/invidious/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/invidious/config/file.sls
